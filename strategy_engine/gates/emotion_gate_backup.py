...
- 结果产出：
    {
      passed: bool,
      reasons: list[str],
      indicators: IndicatorsPayload (step2 · 3的输出，原样透传),
      thresholds: { ... 快照 ... },
      window: "10:00" | "14:00",
      tz: "Asia/Shanghai",
...
- 事件：
  ...
  （若找不到项目内的事件写入封装，则退化写到 events/altflow_events.jsonl）
"""
...
# 优雅对接 storage：now_shanghai（时间戳）一定有；事件写入函数若无则降级到本地 JSONL
try:
...
_EVENT_WRITERS_CANDIDATES: Tuple[str, ...] = (
    "append_event_jsonl",  # 自定义 JSONL 追加
    "write_event",         # 统一事件写入
    "log_event",           # 记录事件
    "append_event",        # 事件追加
)
...
def _event_writer() -> Optional[Callable[[str, Dict[str, Any]], None]]:
    ...
    优先使用 data_service.storage 中的已存在函数；否则回退到本地 JSONL。
    """
    # 优先：项目内封装
...
def check_emotion_gate(
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "passed": passed,
        "reasons": reasons,
        # 为对齐验收样例：sources 放在顶层，指标整体放在 indicators
        "sources": indi.get("sources", {}),
        "indicators": {
            # 保留 step2 · 3 的最小契约：schema_version/calc_at/intraday_missing/metrics/...
