...
6. **调度与工具** – `utils/scheduler.py` 使用 APScheduler 在 Asia/Shanghai 时区内只在 10:00 和 14:00 触发任务，内置交易日历缓存和降级机制；其他工具包括 logger、helpers 等。
...
默认情况下运行 `python -m main` 将启动调度器并维持空循环；定时任务将在每日 10:00 和 14:00 触发策略主循环。使用 `CTRL+C` 退出时，调度器会安全关闭。
...
**声明**：启动或发布该系统仅供学习和研究之用，不构成任何投资建议。